...
class MovementSystem:
    def update(self, entities: list[Entity], delta):
        for entity in entities:
            ...
            self._calculate_direction(
                movement_component, position_component, direction_component
            )
            ...
            self._calculate_velocity(
                delta, movement_component, direction_component, velocity_component
            )
            self._apply_velocity(position_component, velocity_component)
            self._update_move_target(
                movement_component, position_component, velocity_component
            )
    ...
    def _calculate_target_normalized(
        self,
        movement_component: MovementComponent,
        position_component: PositionComponent,
    ):
        ...
        movement_component.distance_to_target = (
            target_vector_x**2 + target_vector_y**2
        ) ** 0.5
        ...
        if movement_component.distance_to_target != 0:
            movement_component.target_normalized_x = (
                target_vector_x / movement_component.distance_to_target
            )
            movement_component.target_normalized_y = (
                target_vector_y / movement_component.distance_to_target
            )
        else:
            movement_component.target_normalized_x = 0
            movement_component.target_normalized_y = 0

    def _apply_steering_force(
        self,
        movement_component: MovementComponent,
        direction_component: DirectionComponent,
    ):
        if (
            ...
            or movement_component.target_normalized_x is None
            or movement_component.target_normalized_y is None
        ):
            return
        ...
        new_dx = (
            (1 - steering_force) * direction_component.normalized_x
            + steering_force * movement_component.target_normalized_x
        )
        new_dy = (
            (1 - steering_force) * direction_component.normalized_y
            + steering_force * movement_component.target_normalized_y
        )
    ...
    def _apply_accelerate_and_decelerate(
        self, delta, movement_component: MovementComponent
    ):
        if movement_component.distance_to_target is None:
            return
        ...
        if (
            movement_component.distance_to_target
            > movement_component.slow_down_distance
        ):
        ...
        current_speed = max(
            movement_component.min_speed, min(current_speed, movement_component.speed)
        )
    ...
    def _calculate_velocity(
        ...
        velocity_component.x = (
            direction_component.normalized_x * movement_component.current_speed * delta
        )
        velocity_component.y = (
            direction_component.normalized_y * movement_component.current_speed * delta
        )
    ...
    def _update_move_target(
        ...
        center_pos = position_component.get_center()
        distance = math.sqrt(
            (movement_component.target_x - center_pos[0]) ** 2
            + (movement_component.target_y - center_pos[1]) ** 2
        )
